...
//Falta ver como corrigir o problema do token que não ta validando
...
describe("POST /credentials", () => {
  ...
  describe("when body is valid", () => {
    ...
    it("should respond with object created", async () => {
      ...
      expect(response.body).toEqual({...newBody,userId: user.id
      });
    });
...
describe("DELETE /credentials/:id", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 400 when id is not a number ", async () => {
      ...
      const token = generateValidToken(user);
      const credential = await createCredential(user.id);
      const response = await server.delete("/credentials/"+(credential.id+1)).set("Authorization", `Bearer ${token}`);
